...
export default function Home() {
  ...
  return (
    <div id="catalogRoot">
      {/* HERO */}
      <section className="hero min-h-[92vh] grid grid-cols-1 lg:grid-cols-2 relative overflow-hidden">
        ...
        <div className="hero-visual relative overflow-hidden hidden lg:block">
          ...
          <div className="hero-wood-texture absolute inset-10 border border-border flex items-center justify-center">
            <div className="wood-rings w-[300px] h-[300px] relative">
      ...
      {/* LANGUAGES */}
      <section className="languages-section px-10 md:px-20 py-20 bg-white border-y border-border">
        <div className="section-eyebrow text-[10px] tracking-[4px] uppercase text-oak mb-4">{t["lang.eyebrow"]}</div>
        <h2 className="section-title font-serif text-[clamp(36px,4vw,56px)] font-light leading-[1.1] text-charcoal mb-12" dangerouslySetInnerHTML={{ __html: t["lang.title"] }} />
        <div className="lang-grid grid grid-cols-2 md:grid-cols-4 bg-border gap-[1px]">
          {[
            { code: 'en', flag: '🇬🇧', name: 'English', native: 'English', sample: '"Solid oak, handcrafted to order. Delivered across Europe."' },
            { code: 'de', flag: '🇩🇪', name: 'Deutsch', native: 'German', sample: '"Massivholz aus Eiche, handgefertigt auf Bestellung."' },
            { code: 'fr', flag: '🇫🇷', name: 'Français', native: 'French', sample: '"Chêne massif, fabriqué à la main sur commande."' },
            { code: 'cs', flag: '🇨🇿', name: 'Čeština', native: 'Czech', sample: '"Masivní dub, ručně vyrobený na zakázku."' },
            { code: 'pl', flag: '🇵🇱', name: 'Polski', native: 'Polish', sample: '"Lity dąb, ręcznie wykonany na zamówienie."' },
            { code: 'it', flag: '🇮🇹', name: 'Italiano', native: 'Italian', sample: '"Rovere massello, lavorato a mano su ordinazione."' },
            { code: 'es', flag: '🇪🇸', name: 'Español', native: 'Spanish', sample: '"Roble macizo, fabricado a mano bajo pedido."' },
            { code: 'nl', flag: '🇳🇱', name: 'Nederlands', native: 'Dutch', sample: '"Massief eikenhout, handgemaakt op bestelling."' },
            { code: 'sk', flag: '🇸🇰', name: 'Slovenčina', native: 'Slovak', sample: '"Masívny dub, ručne vyrobený na objednávku."' },
            { code: 'uk', flag: '🇺🇦', name: 'Українська', native: 'Ukrainian', sample: '"Масив дуба, виготовлено вручну на замовлення."' },
          ].map(l => (
            <div
              key={l.code}
              className={`lang-card p-8 cursor-pointer transition-colors ${lang === l.code ? 'bg-charcoal text-white' : 'bg-white hover:bg-cream'}`}
              onClick={() => setLang(l.code)}
            >
              <span className="lang-flag text-[28px] mb-4 block">{l.flag}</span>
              <div className={`lang-name font-serif text-[20px] font-normal mb-1 ${lang === l.code ? 'text-white' : 'text-charcoal'}`}>{l.name}</div>
              <div className={`lang-native text-[12px] tracking-[1px] ${lang === l.code ? 'text-white/45' : 'text-warm-mid'}`}>{l.native}</div>
              {lang === l.code && (
                <div className="lang-sample mt-4 text-[13px] text-white/60 italic leading-[1.5] animate-fadeUp">{l.sample}</div>
              )}
            </div>
          ))}
        </div>
